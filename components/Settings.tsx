...
const Settings: React.FC<SettingsProps> = ({ state, setState, notify }) => {
  ...
  const handleUpdateList = async (listKey: 'finance' | 'inventory' | 'roles', action: 'add' | 'remove', value: string) => {
    ...
    try {
      ...
      notify("Cadastro Cloud atualizado com sucesso");
    } catch (e: any) { 
      console.error(e);
      notify("Falha crítica ao salvar categoria", "error"); 
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      ...
      await refreshData();
      notify("Meta mensal atualizada no servidor");
    ...
    <div className="space-y-6 pb-24">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Configurações de Listas e Metras Mensais</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-100 p-6 rounded-[32px] space-y-4 shadow-sm hover:shadow-md transition-all">
          <h4 className="text-[10px] font-black uppercase text-emerald-600 flex gap-2 items-center"><Tag size={14}/> Fluxo Financeiro</h4>
          <div className="flex gap-2">
            <input className="flex-1 bg-slate-50 border border-slate-200 p-3 rounded-xl text-[10px] font-black uppercase outline-none focus:border-emerald-500" placeholder="NOVA CATEGORIA..." value={newEntries.finance} onChange={e => setNewEntries({...newEntries, finance: e.target.value})} />
            <button onClick={() => handleUpdateList('finance', 'add', newEntries.finance)} className="bg-slate-900 text-white p-3 rounded-xl hover:bg-emerald-600 transition-all"><Plus size={18}/></button>
          </div>
          <div className="flex flex-wrap gap-2 max-h-48 overflow-auto pt-2 scrollbar-hide">
            {state.financeCategories.map(c => (
              <div key={c} className="bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg text-[8px] font-black flex items-center gap-2 uppercase text-slate-600 group">
                {c} 
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-[32px] space-y-4 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="flex gap-2">
            <input className="flex-1 bg-slate-50 border border-slate-200 p-3 rounded-xl text-[10px] font-black uppercase outline-none focus:border-blue-500" placeholder="NOVA CATEGORIA..." value={newEntries.inventory} onChange={e => setNewEntries({...newEntries, inventory: e.target.value})} />
            <button onClick={() => handleUpdateList('inventory', 'add', newEntries.inventory)} className="bg-slate-900 text-white p-3 rounded-xl hover:bg-blue-600 transition-all"><Plus size={18}/></button>
          </div>
          <div className="flex flex-wrap gap-2 max-h-48 overflow-auto pt-2 scrollbar-hide">
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-[32px] space-y-4 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="flex gap-2">
            <input className="flex-1 bg-slate-50 border border-slate-200 p-3 rounded-xl text-[10px] font-black uppercase outline-none focus:border-indigo-500" placeholder="NOVO CARGO..." value={newEntries.roles} onChange={e => setNewEntries({...newEntries, roles: e.target.value})} />
            <button onClick={() => handleUpdateList('roles', 'add', newEntries.roles)} className="bg-slate-900 text-white p-3 rounded-xl hover:bg-indigo-600 transition-all"><Plus size={18}/></button>
          </div>
          <div className="flex flex-wrap gap-2 max-h-48 overflow-auto pt-2 scrollbar-hide">
            {state.employeeRoles.map(r => (
              <div key={r} className="bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg text-[8px] font-black flex items-center gap-2 uppercase text-slate-600 group">
                {r} 
