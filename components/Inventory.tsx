...
const Inventory: React.FC<InventoryProps> = ({ state, setState }) => {
  ...
  const filteredItems = state.inventory.filter(item => {
    ...
    const matchesStatus = statusFilter === 'all' || (statusFilter === 'critical' && isCritical) || (statusFilter === 'ok' && !isCritical);
    ...
    <div className="space-y-6 pb-24 md:pb-8">
          ...
          <div className="md:col-span-6 space-y-1 relative" ref={dropdownRef}>
            ...
            <div className="relative group">
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
                {selectedItemId && <button onClick={() => { setSelectedItemId(''); setItemSearchText(''); }} className="p-1 hover:bg-slate-200 rounded-full text-slate-400"><X size={14} /></button>}
              ...
              {isDropdownOpen && (
                <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white border border-slate-100 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 max-h-64 overflow-y-auto z-[100]">
                  {filteredDropdownItems.length > 0 ? filteredDropdownItems.map(item => (
                    <button key={item.id} type="button" className={`w-full text-left px-5 py-3.5 flex justify-between items-center transition-colors border-b border-slate-50 last:border-0 hover:bg-blue-50 group ${selectedItemId === item.id ? 'bg-blue-50' : ''}`} onClick={() => { setSelectedItemId(item.id); setItemSearchText(item.name.toUpperCase()); setIsDropdownOpen(false); }}>
                      <div className="flex flex-col"><span className={`text-[11px] font-black uppercase tracking-tight ${selectedItemId === item.id ? 'text-blue-600' : 'text-slate-700'}`}>{item.name}</span><span className="text-[8px] font-bold text-slate-400 uppercase">{item.category}</span></div>
                      <div className="text-right"><span className={`text-[10px] font-black ${item.currentQty <= item.minQty ? 'text-rose-500' : 'text-slate-400'}`}>Saldo: {item.currentQty}</span></div>
                    </button>
                  )) : <div className="p-8 text-center"><p className="text-[10px] font-black text-slate-300 uppercase italic">Nenhum item encontrado</p></div>}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden relative z-0">
        <div className="p-6 border-b border-slate-100 bg-slate-50/30 flex flex-col space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3"><div className="w-8 h-8 bg-slate-900 text-white rounded-lg flex items-center justify-center"><ListFilter size={16} /></div><h3 className="font-black text-sm text-slate-800 uppercase tracking-tight">Saldo em Estoque</h3></div>
            <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/10 w-full md:w-64" placeholder="Pesquisar no saldo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
          </div>
          <div className="flex flex-wrap gap-3 pt-2">
            <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-xl"><Tag size={14} className="text-slate-400" /><select className="bg-transparent text-[10px] font-black uppercase outline-none text-slate-600" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}><option value="all">Todas Categorias</option><option value="peças">Peças</option><option value="insumos">Insumos</option><option value="EPIs">EPIs</option><option value="outros">Outros</option></select></div>
            <div className="flex items-center gap-2 bg-white border border-slate-200 px-3 py-1.5 rounded-xl"><Filter size={14} className="text-slate-400" /><select className="bg-transparent text-[10px] font-black uppercase outline-none text-slate-600" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option value="all">Todos Status</option><option value="ok">Status: OK</option><option value="critical">Status: Crítico</option></select></div>
            {(categoryFilter !== 'all' || statusFilter !== 'all' || searchTerm !== '') && <button onClick={() => { setCategoryFilter('all'); setStatusFilter('all'); setSearchTerm(''); }} className="text-[9px] font-black text-blue-600 uppercase hover:underline">Limpar Filtros</button>}
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 text-[9px] uppercase text-slate-400 font-black border-b border-slate-100 tracking-widest">
              <tr><th className="px-8 py-5">Produto</th><th className="px-8 py-5">Categoria</th><th className="px-8 py-5 text-center">Mínimo</th><th className="px-8 py-5 text-right">Saldo Atual</th><th className="px-8 py-5 text-center">Status</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map(item => {
                const isCritical = item.currentQty <= item.minQty;
                return (
                  <tr key={item.id} className="hover:bg-slate-50/30 transition-colors"><td className="px-8 py-5 font-black text-slate-800 text-sm uppercase">{item.name}</td><td className="px-8 py-5"><span className="text-[10px] font-black text-slate-400 uppercase bg-slate-100 px-2 py-1 rounded-lg">{item.category}</span></td><td className="px-8 py-5 text-center text-slate-400 font-bold">{item.minQty}</td><td className={`px-8 py-5 text-right font-black text-lg ${isCritical ? 'text-rose-600' : 'text-slate-800'}`}>{item.currentQty}</td><td className="px-8 py-5 text-center">{isCritical ? <div className="flex items-center justify-center gap-1.5 text-rose-600 bg-rose-50 px-3 py-1.5 rounded-full inline-flex"><AlertCircle size={12} strokeWidth={3} /><span className="text-[8px] font-black uppercase tracking-widest">Crítico</span></div> : <div className="flex items-center justify-center gap-1.5 text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full inline-flex"><CheckCircle2 size={12} strokeWidth={3} /><span className="text-[8px] font-black uppercase tracking-widest">OK</span></div>}</td></tr>
                );
              })}
            </tbody>
          </table>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[9px] uppercase text-slate-400 font-black border-b border-slate-100 tracking-widest">
              <tr><th className="px-8 py-4">Data</th><th className="px-8 py-4">Item</th><th className="px-8 py-4">Qtd</th><th className="px-8 py-4">Operação</th><th className="px-8 py-4 text-center">Ações</th></tr>
            </thead>
              ...
              {state.inventoryExits.slice(-10).reverse().map(exit => {
                ...
                return (
                  <tr key={exit.id} className="hover:bg-slate-50/50">
                    ...
                    <td className={`px-8 py-4 font-black ${isOut ? 'text-rose-600' : 'text-emerald-600'}`}>{isOut ? '-' : '+'}{exit.quantity}</td>
                    <td className="px-8 py-4"><span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${isOut ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>{exit.destination}</span></td>
                    <td className="px-8 py-4 text-center"><button onClick={() => setConfirmReversal({ isOpen: true, movementId: exit.id })} className="p-2 text-slate-300 hover:text-rose-500 transition-all" title="Estornar"><Trash2 size={16} /></button></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[120] flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200 p-8 space-y-6">
            <div className="flex justify-between items-center"><h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Cadastrar Novo Produto</h3><button onClick={() => setShowAddForm(false)} className="p-2 text-slate-400 hover:bg-slate-100 rounded-full transition-all"><X /></button></div>
